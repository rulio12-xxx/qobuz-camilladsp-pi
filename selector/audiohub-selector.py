...
CONFIG = {
    ...
    "Fichiers":  f"{CFG_DIR}/camilladsp-fichiers.yml",
}
...
class MpdSource(Source):
    """MPD : fichiers locaux et radios, sous-flux 1 du loopback."""

    def raw_active(self):
        try:
            txt = open(MPD_STATUS).read()
        except FileNotFoundError:
            return False
        return "state: RUNNING" in txt
...
def main():
    ...
    fichiers = MpdSource("Fichiers", priority=-1)
    sources = [fichiers, qobuz]
    if ENABLE_SPDIF:
        sources += [
